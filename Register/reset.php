...
if(isset($_POST['submitRecup'])){
	$goodemail="";	
	if($_POST["email"]!='root@localhost.com'){
	if(checkEmailExists($_POST["email"])){
		$goodemail=true;
		
		setNewHashToEmail($_POST["email"]);
		
		sendEmailto($_POST["email"],gethashFromEmail($_POST["email"]));
...
function sendEmailto($emailSend,$hash){
	...
	$mail->Subject = "Recuperação de conta @ SDL Platform";
	$mail->Body = "Altere a sua password aqui:
			 <a href='https://seekdataleakage.ddns.net/SDLPlatform/Register/resetPassword.php?email=".$emailSend."&hash=".$hash."'>Reset password</a>
...
<li><label for="signup"><i class="fa fa-credit-card"></i> Recuperar conta</label></li>
</ul>
</div>
...
<li><span class="icon"><i class="fa fa-envelope-o"></i></span><input type="email" required class="input" id="email" name="email" placeholder="Email a verificar"value=""/></li>
<li><input name="submitRecup" type="submit" value="Recuperar" class="btn"></li>
</ul>
</form>
<?php if(isset($_POST['email'])){?>
<?php if($goodemail==true){?>
<p style="text-align:center; font-size:13px; color:#007F00;">Foi enviado um email com as instru&ccedil;&otilde;es de reset da password.</p>
<?php }else{?>
<p style="text-align:center; font-size:13px; color:#d62e2e;"> Erro! O email que inseriu n&atildeo existe. </p>
<?php }}?>
